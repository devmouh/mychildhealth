...
class CompleteConsultationSerializer(serializers.Serializer):
    """
    Validates the full payload sent by the doctor to complete a consultation.
    Body:
    {
      ...
      "traitements":  [ { "medicament": "Ventoline", "dose": "2 puffs", "duree": "7j", "instructions": "..." } ]
    }
    """
    consultation = ConsultationInputSerializer()
    diagnostic   = DiagnosticInputSerializer()
    traitements  = TraitementInputSerializer(many=True, required=False)